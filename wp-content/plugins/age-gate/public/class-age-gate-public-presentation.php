...
class Age_Gate_Presentation extends Age_Gate_Public
{
    ...
    public function enqueue_scripts()
    {
        ...
        if ($this->settings['advanced']['use_js']) {
            ...
            if ($this->settings['advanced']['disable_right']) {
                wp_enqueue_script($this->plugin_name . '-interaction', plugin_dir_url(__FILE__) . 'js/interactions.js', array(), $this->version);
            }
    ...
    {
        if ($this->settings['advanced']['preload_images']) {
            if ($this->settings['appearance']['logo']) {
                echo sprintf('<link rel="preload" href="%s" as="image" />', wp_get_attachment_url($this->settings['appearance']['logo']));
                echo "\r\n";
            }
            if ($this->settings['appearance']['background_image']) {
                $file = wp_get_attachment_url($this->settings['appearance']['background_image']);
                $type = wp_check_filetype($file);

                if (strpos($type['type'], 'image') !== false && $this->settings['appearance']['logo'] !== $this->settings['appearance']['background_image']) {
    ...
    {
        $css = ':root{--age-gate-blur: '. (isset($this->settings['appearance']['blur_amount']) && $this->settings['appearance']['blur_amount'] ? $this->settings['appearance']['blur_amount'] : 0) .'px;}';
