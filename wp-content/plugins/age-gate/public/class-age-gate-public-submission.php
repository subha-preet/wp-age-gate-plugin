...
class Age_Gate_Submission extends Age_Gate_Public
{
    ...
    private function _set_cookie($age, $remember)
    {
        ...
        if ($set_cookie) {
            ...
            setcookie($cookieName, abs(ceil($age)), $length, COOKIEPATH, apply_filters('age_gate/cookie_domain', COOKIE_DOMAIN));
        }
        // die();
    }
    ...
    private function _set_error_message($message, $data = array())
    {
        ...
        if ($set_cookie) {
            setcookie('age_gate_error', $id, 0, COOKIEPATH, apply_filters('age_gate/cookie_domain', COOKIE_DOMAIN));
        }
    }
}
